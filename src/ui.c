 ...
 *
 * This function creates a temporary overlay window to display a list of
 * application keybindings. The content of the help window changes based
 * on the current `view_mode` (archive navigation or content viewing).
 * It waits for a key press to close the help window.
 *
 * @param state A read-only pointer to the current application state, used to determine view mode.
 */
void ui_show_help(const AppState* state) {
    ...
    wbkgd(help_win, COLOR_PAIR(COLOR_PAIR_STATUSBAR)); // Set background color
    wattron(help_win, COLOR_PAIR(COLOR_PAIR_HELP_BORDER));
    box(help_win, 0, 0); // Draw a border around the help window
    ...
    if (state->view_mode == VIEW_MODE_ARCHIVE) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            mvwprintw(help_win, i + 3, 3, "%-12s", keys[i][0]);
    ...
    wrefresh(help_win);   // Display the help window
    getch();              // Wait for a key press to close
    delwin(help_win);     // Delete the help window
